...
if [ ! -f "$DEPLOY_DIR/.env" ]; then
    if [ -f "$DEPLOY_DIR/config/env.sample" ]; then
        ...
        echo "Created .env file from template. Please check and update any values as needed."
        read -p "Would you like to edit the .env file now? (y/n) " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ${EDITOR:-nano} "$DEPLOY_DIR/.env"
        else
            echo "Continuing with the pre-filled values in .env"
        fi
    else
...
server {
    listen 80;
    server_name ${SERVER_NAME:-themis-staging.example.com};
...
# Set default directories for client and server
DEFAULT_CLIENT_DIR="/path/to/themis-client"
DEFAULT_SERVER_DIR="/path/to/themis-server"

# 6. Deploy application code (client)
echo "Step 6/7: Deploying client application..."
read -p "Enter the path to your themis-client directory [$DEFAULT_CLIENT_DIR]: " CLIENT_DIR
CLIENT_DIR=${CLIENT_DIR:-$DEFAULT_CLIENT_DIR}
...
if [ -d "$CLIENT_DIR" ]; then
    ...
    echo "Client application deployed successfully."
else
    echo "Error: Client directory not found at $CLIENT_DIR."
    echo "Please provide the correct path to your themis-client directory."
    ...
    if [ -d "$CLIENT_DIR" ]; then
        ...
        echo "Error: Client directory still not found. Skipping client deployment."
        echo "You will need to deploy the client manually."
    fi
fi

# 7. Deploy and start server
echo "Step 7/7: Deploying and starting server..."
read -p "Enter the path to your server directory [$DEFAULT_SERVER_DIR]: " SERVER_DIR
...
if [ -d "$SERVER_DIR" ]; then
    ...
    pm2 save
    pm2 startup
    echo "Server deployed and started successfully."
else
    echo "Error: Server directory not found at $SERVER_DIR."
    echo "Please provide the correct path to your server directory."
    ...
    if [ -d "$SERVER_DIR" ]; then
        ...
        echo "Error: Server directory still not found. Skipping server deployment."
        echo "You will need to deploy the server manually."
    fi
fi
...
echo "http://${SERVER_NAME:-themis-staging.example.com}"
echo ""
...
echo "Then run: sudo certbot --nginx -d ${SERVER_NAME:-themis-staging.example.com}"
echo ""
