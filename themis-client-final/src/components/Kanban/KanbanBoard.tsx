...
const KanbanBoard: React.FC<KanbanBoardProps> = ({ 
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    // If there is no destination or if the task was dropped in the same place
    if (!destination || 
    ...
    // Extract task status from the droppable ID (which is the column status)
    const newStatus = destination.droppableId as TaskStatus;
    
    // Call the onTaskUpdate callback with the task ID and new status
    if (onTaskUpdate) {
